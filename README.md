...
[![Latest release](https://img.shields.io/github/v/release/singledevai-droid/openclaw-quota-pilot?cacheSeconds=300)](https://github.com/singledevai-droid/openclaw-quota-pilot/releases/latest)
...
active terminal, and lets every agent use a shared OAuth pool while retaining
an independent routing mode.
...
```text
$(dashboard) main · Work account · 5h 82% · W 37% · PROFILE PINNED
```

Click it to:

- choose or automatically follow an OpenClaw agent;
- choose the exact main, Telegram, heartbeat, or automation session;
- refresh quota;
...
`PROFILE PINNED`, `PROFILE AUTO`, and `PROFILE FALLBACK` describe profile
routing for the session shown in the menu, not agent detection. `PINNED` is an
explicit user override, `AUTO` is managed by Quota Pilot, and `FALLBACK` follows
OpenClaw's native profile order. A manual agent or session choice is scoped to
the active terminal tab. Choose
...
terminal tabs after an agent ID or a unique prefix (for example, `content` for
`content-agent`). The extension then falls back to terminal cwd and the open
workspace.
