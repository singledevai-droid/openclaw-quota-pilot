...
| `credentialOwnerAgentId` | `agentId` | Canonical agent whose OAuth store receives reauthorization. Use `main` for a shared pool. |
...
| `quotaPilot.showMode` | `true` | Show `PROFILE AUTO`, `PROFILE PINNED`, or `PROFILE FALLBACK`. |

Leave `quotaPilot.sessionKey` empty unless the default agent needs a custom
session. Agent selection starts at `agent:<agentId>:main`; the menu can then
target any stored main, Telegram, heartbeat, or automation session explicitly.
