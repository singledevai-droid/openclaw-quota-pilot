...
function toQuickPickItems(
  ...
  return [
    ...
    {
      label: `$(comment-discussion) Session: ${status.sessionKey}`,
      description: status.routingMode.toUpperCase(),
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const terminalOverrides = new WeakMap<vscode.Terminal, RouteTarget>();
  const oauthTerminals = new WeakMap<vscode.Terminal, RouteTarget>();
  let windowOverrideTarget: RouteTarget | null = null;
  ...
  const manualOverrideAgentId = (): string | null => {
    ...
    return terminal
      ? terminalOverrides.get(terminal)?.agentId ?? null
      : windowOverrideTarget?.agentId ?? null;
  };

  const manualOverrideTarget = (): RouteTarget | null => {
    const terminal = vscode.window.activeTerminal;
    return terminal ? terminalOverrides.get(terminal) ?? null : windowOverrideTarget;
  };
  ...
  const currentTarget = (): RouteTarget => {
    ...
    if (override?.agentId === agentId) return override;
    return {
      agentId,
      sessionKey:
  ...
  const detectCurrentAgent = async (refreshStatus: boolean): Promise<void> => {
    ...
    if (nextAgentId === detectedAgentId && nextSource === detectionSource) {
      ...
      if (
        refreshStatus &&
        (!latestStatus || targetCacheKey(latestStatus) !== targetCacheKey(target))
      ) {
        latestStatus = null;
        await update(false);
      }
  ...
  const resumeAgentDetection = async (): Promise<void> => {
    ...
    else windowOverrideTarget = null;
  ...
  const selectAgent = async (): Promise<void> => {
    ...
    const target = {
      ...
      sessionKey: choice.agent.sessionKey || `agent:${choice.agent.agentId}:main`,
    };
    if (activeTerminal) terminalOverrides.set(activeTerminal, target);
    else windowOverrideTarget = target;
    applyCachedStatusForCurrentTarget();
    await vscode.window.withProgress(
      {
        location: vscode.ProgressLocation.Notification,
  ...
  const selectSession = async (): Promise<void> => {
    ...
    try {
      sessions = await vscode.window.withProgress(
        {
          location: vscode.ProgressLocation.Notification,
          title: `Quota Pilot: loading ${agentId} sessions…`,
        },
        () => client.sessions(agentId),
      );
    } catch (error) {
      applyError(error, true);
      return;
    ...
    const choice = await vscode.window.showQuickPick(
      sessions.map((session) => ({
        label: `${
          session.sessionKey === current.sessionKey
            ? "$(check)"
            : "$(comment-discussion)"
        } ${session.label ?? session.kind}`,
        description: [
          session.kind.toUpperCase(),
          session.profileSource?.toUpperCase(),
          session.updatedAt
      ...
      {
        title: `Select ${agentId} session`,
        placeHolder: "Profile changes apply only to the selected session",
        matchOnDescription: true,
        matchOnDetail: true,
      },
    );
    if (!choice) return;
    const target = {
      agentId: choice.session.agentId,
      sessionKey: choice.session.sessionKey,
    };
    const activeTerminal = vscode.window.activeTerminal;
    if (activeTerminal) terminalOverrides.set(activeTerminal, target);
    else windowOverrideTarget = target;
  ...
  const showMenu = async (): Promise<void> => {
    ...
    try {
      ...
      if (selection.action === "select-agent") {
        await selectAgent();
      } else if (selection.action === "select-session") {
        await selectSession();
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("quotaPilot.selectSession", selectSession),
