...
export function formatStatusBar(status: PilotStatus, showMode: boolean): string {
  ...
  if (showMode) {
    const routingMode =
      status.routingMode ?? (status.mode === "auto" ? "auto" : "fallback");
    parts.push(`PROFILE ${routingMode.toUpperCase()}`);
  }
  return parts.join(" · ");
}
...
export function buildTooltipMarkdown(status: PilotStatus, now = Date.now()): string {
  ...
  const lines = [
    ...
    "",
    `Profile routing: **${routingMode.toUpperCase()}**  \\`,
    `Session: \`${status.sessionKey}\`  \\`,
    `Credentials: \`${status.credentialOwnerAgentId ?? status.agentId}\`  \\`,
