...
const checks = [
  ...
  ["non-public fixture alphabet", /[\u0400-\u04ff]/],
];
const publicFixtureAgentIds = new Set(["main", "content-agent", "support-agent"]);
...
async function walk(directoryUrl) {
  for (const entry of await readdir(directoryUrl, { withFileTypes: true })) {
    ...
    for (const match of content.matchAll(/\bagent:([a-z0-9-]+):/g)) {
      if (!publicFixtureAgentIds.has(match[1])) {
        failures.push(`${path}: non-generic fixture agent ID`);
      }
    }
  }
}
